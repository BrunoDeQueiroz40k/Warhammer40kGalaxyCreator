...
import { Camera, Download, FileDown, FileUp, Sparkles } from "lucide-react";

import { AuthUser } from "../../hooks/useAuth";
import { useUserProfileModalData } from "../../hooks/useUserProfileModalData";
import { formatDateTime } from "../../ts/functions";
          ...
          <div className="relative">
            ...
            <div className="px-6 md:px-8 pb-8">
              ...
              <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-3">
                ...
                <div className="rounded-lg border border-amber-500/25 bg-black/60 p-4 text-center">
                  <p className="text-xs text-slate-300">Importações</p>
                  <p className="text-3xl font-semibold mt-1">{imports.length}</p>
                </div>
                <div className="rounded-lg border border-amber-500/25 bg-black/60 p-4 text-center">
                  <p className="text-xs text-slate-300">Exportações</p>
                  <p className="text-3xl font-semibold mt-1">{exportsHistory.length}</p>
                </div>
              </div>

              <div className="mt-4 grid grid-cols-1 lg:grid-cols-12 gap-4">
                <section className="lg:col-span-4 rounded-lg border border-amber-500/25 bg-black/60 p-4">
                ...
                <section className="lg:col-span-4 rounded-lg border border-amber-500/25 bg-black/60 p-4">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <h3 className="font-semibold">Histórico de importação</h3>
                  </div>
                  <div className="space-y-2 max-h-80 overflow-y-auto pr-1">
                    {!importsLoaded && (
                      ...
                      >
                        {loadingImports ? "Carregando..." : "Carregar histórico"}
                      </Button>
                    )}
                    {imports.length === 0 && (
                      importsLoaded ? (
                        <p className="text-xs text-slate-400">Nenhuma importação registrada.</p>
                      ) : null
                    )}
                    {imports.map((item) => (
                      <div key={item.id} className="rounded-md border border-amber-500/20 bg-black/25 p-3">
                        <p className="text-sm text-slate-100 truncate">{item.fileName}</p>
                        <p className="text-xs text-slate-300 mt-0.5">{item.planetCount} planetas</p>
                        <p className="text-[11px] text-slate-400 mt-1">{formatDateTime(item.createdAt)}</p>
                      </div>
                    ))}
                  </div>
                </section>
                ...
                <section className="lg:col-span-4 rounded-lg border border-amber-500/25 bg-black/60 p-4">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <h3 className="font-semibold">Histórico de exportação</h3>
                  </div>
                  <div className="space-y-2 max-h-80 overflow-y-auto pr-1">
                    {!exportsLoaded && (
                      <Button
                      ...
                      >
                        {loadingExports ? "Carregando..." : "Carregar histórico"}
                      </Button>
                    )}
                    {exportsHistory.length === 0 && (
                      exportsLoaded ? (
                        <p className="text-xs text-slate-400">Nenhuma exportação registrada.</p>
                      ) : null
                    )}
                    {exportsHistory.map((item) => (
                      <div
                        key={item.id}
                        className="rounded-md border border-amber-500/20 bg-black/25 p-3 flex items-center justify-between gap-2"
                      >
                        <div className="min-w-0">
                          <p className="text-sm text-slate-100">{item.planetCount} planetas</p>
                          <p className="text-[11px] text-slate-400 mt-1">{formatDateTime(item.createdAt)}</p>
                        </div>
                        <Button
                          variant="accept"
                          className="px-2 py-1"
                          disabled={!item.snapshotId || downloadingSnapshotId === item.snapshotId}
                          onClick={() => void handleExportRedownload(item.snapshotId)}
                        >
                          <Download className="w-4 h-4" />
                          {downloadingSnapshotId === item.snapshotId ? "Baixando..." : "Baixar"}
                        </Button>
                      </div>
                    ))}
                  </div>
